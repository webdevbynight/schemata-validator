...
it.each(mockedInvalidJSONLDData)(
  "should throw an error if the input is not a valid JSON-LD string",
  input => {
    expect(() => new JSONLDParser(input).parse()).toThrow();
  }
);
it.each(mockedJSONLDData)(
  "should return the intermediate representation tree for data %$",
  ({ input, expectedTree }) => {
